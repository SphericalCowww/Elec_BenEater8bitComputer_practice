...
////instruction map
uint16_t instructionDef[] = {
  MI|CO, RO|II|CE, 0,     0,     0,        0, 0, 0, //0000 - NOP (no operation)
  MI|CO, RO|II|CE, IO|MI, RO|AI, 0,        0, 0, 0, //0001 - LDA (load register A from RAM)
  MI|CO, RO|II|CE, IO|MI, RO|BI, EO|AI,    0, 0, 0, //0010 - ADD (adding A and B)
  MI|CO, RO|II|CE, IO|MI, RO|BI, EO|AI|SU, 0, 0, 0, //0011 - SUB (subtracting A from B)
...
void setup() {
  ...
  for(int addrVal = 0; addrVal < sizeof(instructionDef)/sizeof(instructionDef[0]); addrVal += 1) {
    writeEEPROM(addrVal, (instructionDef[addrVal] >> 8));
//    writeEEPROM(addrVal, 0);
    if (addrVal % 64 == 0){
      Serial.println(addrVal);
    }
  }
  for(int addrVal = 128; addrVal < sizeof(instructionDef)/sizeof(instructionDef[0])+128; addrVal += 1) {
    writeEEPROM(addrVal, instructionDef[addrVal]);
//    writeEEPROM(addrVal, 0);
    if (addrVal % 64 == 0){
      Serial.println(addrVal);
    }
